...
//    }
    ...
    vTaskDelay(pdMS_TO_TICKS(250)); //Update the display every 500ms
  }
}
...
//If we ask for something like myGNSS.getSIV(), there is no blocking wait, we simply get the last reported value
...
void iconBluetoothTask(void *e)
{
  while (1)
  {
    //Blink BT icon every 500ms
    oled.drawIcon(4, 0, BT_Symbol_Width, BT_Symbol_Height, BT_Symbol, sizeof(BT_Symbol), true);
    vTaskDelay(pdMS_TO_TICKS(500)); 
    oled.rectFill(4, 0, BT_Symbol_Width, BT_Symbol_Height, BLACK, NORM); //erase icon
    vTaskDelay(pdMS_TO_TICKS(500));
  }
}
